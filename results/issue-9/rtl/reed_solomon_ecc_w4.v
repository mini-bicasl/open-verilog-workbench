// Reed-Solomon ECC sub-module: 4-bit data width
...
//   [7:0]  – data byte 0 (4-bit input zero-extended to 8 bits)
//   [15:8] – parity byte 0
//   [23:16]– parity byte 1
//   [31:24]– parity byte 2
//   [39:32]– parity byte 3
...
module reed_solomon_ecc_w4 (
    ...
    input  wire [3:0]  data_in,       // 4-bit data word (upper nibble zero-extended internally)
    ...
    output reg  [3:0]  data_out,      // Extracted data from codeword_in[3:0]
    ...
    assign msg_bytes[0] = { 4'b0, data_in[3:0] };  // Zero-extend 4-bit input to one GF byte

    // --- Encoder: GF(2^8) polynomial remainder (combinational) ---
    // Each mul_0_N is the generator-polynomial coefficient N multiplied by feedback_0.
    // The XOR logic implements GF(2^8) multiplication modulo 0x11D.
    ...
    wire [7:0] mul_0_3;
    // --- Parity bytes (little-endian GF polynomial remainder result) ---
    // parity_byte_0 is byte 1 (codeword[15:8]), parity_byte_3 is byte 4 (codeword[39:32]).
    ...
    // Each syndrome Si = sum_j codeword_byte_j * alpha^(FCR+i*j), evaluated over GF(2^8).
    // syn_mul_i_j is the running Horner-step value; syn_sum_i_j accumulates XOR with next byte.
    // A non-zero syndrome indicates at least one symbol error.
    wire has_error;
